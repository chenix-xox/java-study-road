...
此时请求3访问/3，因为已经有两个并发线程了，他将等待1s，1s后没有执行就进行服务降级

**踩坑：**
设置openfeign的连接超时和读取超时时间为3s，而/9999需要等待5s，因此不论如何请求都是有问题的
...
1&2等待3s，3直接降级（排队呢，线程池已经存在两个线程）

**线程池隔离就是根据线程池已有的数量进行判断，如果已满，将直接进行服务降级**
